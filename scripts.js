...
function gameOver()
{
    ...
    const gameOverMessage = `Vége a játéknak! Eredmény - <span class = 'badge'>${score}</span> Játék újraindítása`
...
{
    calculateScore()
    updateStatusElement(scoreElem, "block", primaryColor, `Találat <span class='badge'>${score}</span>`)
...
function outputChoiceFeedBack(hit)
{
    if(hit)
    {
        updateStatusElement(currentGameStatusElem, "block", winColor, "Talált! :)")
    }
    else
    {
        updateStatusElement(currentGameStatusElem, "block", loseColor, "Nem talált! :(")
    }
}
...
function checkForIncompleteGame()
{
    ...
    if(serializedGameObj)
    {
        ...
        {
            if(confirm('Szeretnéd az előző játékot folytatni?'))
            {
                score = gameObj.score
...
function initializeNewGame(){
    ...
    updateStatusElement(scoreElem,"block",primaryColor,`Találat <span class='badge'>${score}</span>`)
    updateStatusElement(roundElem,"block",primaryColor,`Körök <span class='badge'>${roundNum}</span>`)
...
function initializeNewRound()
{
    ...
    updateStatusElement(currentGameStatusElem, "block", primaryColor, "Keverés...")
...
function shuffleCards()
{
    ...
    function shuffle()
    {
        ...
        if(shuffleCount == 500)
        {
            ...
            dealCards()
            updateStatusElement(currentGameStatusElem, "block", primaryColor, "Találd meg az ászt!")
